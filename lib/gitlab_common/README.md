...
| `ensure_user_projects(gl, user_id, templates)` | Create the two projects every DTaaS user gets, `common` and `user`, each from its own template repository (`ProjectTemplates`, which also carries `import_timeout`, the minutes one import may take, default `IMPORT_TIMEOUT_MINUTES`). Returns `(ok, messages)`; nothing is printed, so each consumer decides what reaches its console. |
